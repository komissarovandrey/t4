...
/**
 ...
 для проверки работо способности программы:

 в stdin подается отсортированный массив, число относительно которого проверяем массив, идет как единственный параметр через консоль
 результат будет выдан в stdout в виде числа
 
 приношу извинения за неправильное имя функции. в моей библиотеки имеется функция с именем bsearch, что мешает компиляции
 ...
 */
int bsearc(int *az,int a,int b,int x,int n)
{
    if ((b-a)>1)
    {
        int c=(a+b)/2;
...
{
    int i=0,j=0,k=8;
    int*az=(int*)malloc(sizeof(int)*k);
    while (scanf("%d",&j)>0)
    {
        az[i]=j;
        i++;
        if(i==k)
        {
            az=(int*)realloc(az,sizeof(int)*(k+=k));
        }
    }
    az[i]=0;
    *n=i;
...
int main(int c,char**stt)
{
    ...
    int n[1];
    int*az=rrr(n);
    printf("%d\n",bsearc(az,0,n[0],atoi(stt[1]),n[0]));
    free(az);
    return 0;
}
/*==================================main======================================*/